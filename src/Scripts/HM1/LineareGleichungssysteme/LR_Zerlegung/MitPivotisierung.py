...
def swapL(matrix, von, zu):
    matrixCopy = np.copy(matrix)
    for col in range(0, von):
        matrix[zu][col] = matrixCopy[von][col]
        matrix[von][col] = matrixCopy[zu][col]
    return matrix
...
def LR_zerlegung_PLR(Matrix):
    ...
    for zeile in range(n - 1):
        ...
        pivot = pivot_suchen(R, zeile)

        # Zeilen tauschen anhand errechnetem Pivot
        swap(R, zeile, pivot)
        swap(P, zeile, pivot)
        swapL(L,zeile, pivot)
...
def lrZerlegungMitPivotisierung(A,b):
